...
char password[] = "petardas";  // your network key
...
void setup() {
  ...
  display.drawString(64, 18, F("By Pelopin"));
  display.display();
...
void displayHolding(int index, int prueba) {
  ...
  if ((timeNow > screenChangeDue))  {
    ...
    if (currentIndex > -1) {
      if (loadDataForHolding(currentIndex)) {
        displayHolding(currentIndex,24);
        delay(3000);
        displayHolding(currentIndex,1);
      } else {
